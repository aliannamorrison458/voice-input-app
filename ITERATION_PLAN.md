...
### 打磨9: UX-4 可发现性 - 第二轮 (2026-04-15)
- 菜单项添加 tooltip (如 "按住 Fn 键快速开始录音", "将上次识别的文字粘贴到当前光标位置")
- 「粘贴上次结果」菜单添加 Cmd+V 快捷键标记
- 录音菜单文案简化（移除括号中的快捷键提示，改用 tooltip）

### 打磨10: UX-5 设置体验 - 第二轮 (2026-04-15)
- 每次打开设置窗口时重新加载当前配置（避免显示过期状态）
- 复用已有设置窗口时刷新 SwiftUI 内容视图
- 窗口尺寸修正为 480x440 与 SettingsView 匹配

### 打磨11: UX-1 视觉与动效 - 第三轮 (2026-04-15)
- 识别成功时菜单栏图标短暂闪现 ✅ (1秒)，给用户即时完成感
- resetUI() 时服务状态显示当前连接地址
- 统一移除菜单项中的 "(Fn)" 后缀（快捷键通过 tooltip 提示）
