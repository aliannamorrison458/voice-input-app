...
### 打磨12: UX-2 交互流畅度 - 第三轮 (2026-04-15)
- 空结果不再弹出通知警告，改为菜单栏显示 🤷 图标 (2秒后恢复)
- 菜单状态栏提示 "🤷 没有识别到文字，可以重试"
- 降低用户因"空结果"产生的焦虑感，鼓励重试

### 打磨13: UX-3 错误与恢复 - 第三轮 (2026-04-15)
- 辅助功能权限被拒时，错误提示更详细：
  - 告知用户结果已保存
  - 引导用户可通过「📋 粘贴上次结果」手动粘贴
  - "取消"按钮改为"稍后设置"，语气更友好

### 打磨14: UX-4 可发现性 - 第三轮 (2026-04-15)
- 辅助功能权限弹窗中明确说明 "按住 Fn 键说话后，松开即可自动输入文字"
- 用户即使不了解 VoiceInput 工作方式，也能从权限提示中学到操作方法

### 打磨15: UX-5 设置体验 - 第三轮 (2026-04-15)
- 保存设置后更新 SettingsWindowController 中的 currentConfig
- 确保再次打开设置窗口时显示的是最新配置
- currentConfig 改为 var 以支持更新
